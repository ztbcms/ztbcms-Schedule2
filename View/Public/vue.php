...
<script>
    window.config = {
        url: {
            ...
            editContent:"{:U('editContent')}",
            delScheduleAcquiescent:"{:U('delScheduleAcquiescent')}",

            scheduAddEdit:"{:U('scheduAddEdit')}",   //添加和编辑模板信息
            rulesAddEdit:"{:U('rulesAddEdit')}",  //添加和编辑模板规则
            scheduleDetails:"{:U('scheduleDetails')}", //添加规则详情
            scheduleList: "{:U('scheduleList')}", //规则模板列表
            queryRule:"{:U('queryRule')}"  //查询时间段内所有的预约
        }
    };
    window.__baseFilters = {
        filters: {
            getPayNameByCode: function (value) {
                var pay = {cod: '现金支付', 'wxpay': '微信支付', 'alipay': '支付宝支付','integral': '积分兑换'};
                return pay[value];
            },
            getFormatTime: function (value) {
                ...
                res += '  ' + (h < 10 ? '0' + h : h) + ':' + (min < 10 ? '0' + min : min) + ':' + (s < 10 ? '0' + s : s);
                return res;
            },
            //不显示时分秒
